...
describe('AuthMiddleware', () => {
  ...
  it('deve permitir requisição com token válido e anexar uid', async () => {
    ...
    expect(req.user).toEqual({ uid: 'user-123', admin: false });
    expect(next).toHaveBeenCalled();
  });

  it('deve anexar flag admin quando o token possui custom claim admin', async () => {
    req.headers = { authorization: 'Bearer token-admin' };
    verifyIdTokenMock.mockResolvedValue({ uid: 'user-123', admin: true });

    await authMiddleware(req, res as Response, next);

    expect(req.user).toEqual({ uid: 'user-123', admin: true });
    expect(next).toHaveBeenCalled();
  });

  describe('adminAuthMiddleware', () => {
    beforeEach(() => {
      req = { headers: {} } as AuthenticatedRequest;
      res = {
        status: jest.fn().mockReturnThis(),
        json: jest.fn().mockReturnThis(),
      };
    ...
    it('deve permitir requisição com usuário admin', () => {
      req.user = { uid: 'admin-123', admin: true };
    ...
    it('deve rejeitar requisição com usuário não-admin', () => {
      req.user = { uid: 'user-123', admin: false };
    ...
    it('deve rejeitar requisição sem usuário autenticado', () => {
      req.user = undefined as unknown as { uid: string; admin: boolean };
