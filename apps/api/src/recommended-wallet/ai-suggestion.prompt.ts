...
Responda APENAS JSON válido no schema {"summary":string,"items":[{"ticker":string,"action":"buy"|"hold"|"reduce","priority":integer,"rationale":string,"suggestedAmount"?:number,"fallbackAllocations"?:[{"ticker":string,"amount":number}]}],"disclaimer":string}.
...
Tickers com qualifiedInvestor=sim são exclusivos para investidor qualificado e OPCIONAIS: podem não estar disponíveis na corretora do usuário. Nunca concentre o aporte apenas neles.
Para cada item qualifiedInvestor=sim com ação "buy", preencha "fallbackAllocations" redistribuindo integralmente o seu suggestedAmount entre tickers com qualifiedInvestor=não e status diferente de "extra", priorizando os abaixo do peso recomendado e depois proporcionalmente ao peso; a soma dos amounts deve ser igual ao suggestedAmount. Inclua em "fallbackAllocations" apenas tickers cujo amount compre ao menos 1 cota (amount >= closePrice); se necessário, concentre o valor em menos tickers. Mencione no rationale que existe alternativa caso não possa comprar.
Mesmo que todos os tickers não qualificados estejam acima do peso, eles continuam elegíveis para receber o valor redistribuído.
